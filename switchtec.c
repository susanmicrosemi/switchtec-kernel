		     ...
		     void *src, size_t size)
{
	...
	stuser->cmd = (tag<<17)|MRPC_GAS_READ;
	tag++;
	memcpy(stuser->data, &offset, sizeof(offset));
	dev_dbg(&stdev->dev, "%s: %x\n",  __func__, *(u32 *)stuser->data);
	memcpy(stuser->data + sizeof(offset), &size, sizeof(size));
	dev_dbg(&stdev->dev, "%s: %x\n",  __func__, *(u32 *)(stuser->data + 4));
...
dev_dbg(&stdev->dev, "offset %x, val %x\n", offset, *(u32 *)dest);
...
static void mrpc_complete_cmd(struct switchtec_dev *stdev)
{
	...
	if (stdev->dma_mrpc) {
		...
		if (stuser->cmd != stdev->dma_mrpc->cmd_id)
...
static ssize_t io_string_show(struct switchtec_dev *stdev, char *buf, 
			      void __iomem *attr, size_t len)
{
	int i;
	const struct switchtec_ops *ops = stdev->ops;
